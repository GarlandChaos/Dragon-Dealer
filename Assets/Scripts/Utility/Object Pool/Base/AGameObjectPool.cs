...
namespace Game.Utility.Pool
{
    ...
    {
        //Object data
        private IObjectPool<T> pool = null; //Game object pool.

        [Header("Pool Data")]
        [SerializeField] private int defaultCapacity = 10; //The default capacity the stack will be created with.
        [SerializeField] private int maxSize = 10; //The maximum size of the pool.
        [SerializeField] private bool collectionCheck = false;  //Collection checks are performed when an instance is returned back to the pool.
                                                                //An exception will be thrown if the instance is already in the pool.
                                                                //Collection checks are only performed in the Editor.

        [Header("Asset References")]
        [SerializeField] private T gameObjectPrefab = null;

        //Properties
        public virtual IObjectPool<T> Pool //Game object pool property
        {
            get
            {
                if (pool == null)
